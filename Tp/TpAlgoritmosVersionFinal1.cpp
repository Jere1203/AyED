...
*/
    ...
    else
    {
        for(int i=0;i<m;i++)
        {
            fprintf(moto,"%d",vectorMot[i].dni);
            fprintf(moto,"%s",vectorMot[i].nombreYapellido);
            fprintf(moto,"%d",vectorMot[i].patente);
            fprintf(moto,"%d",vectorMot[i].codZona);
        }
    }
    fclose(moto);
    return 0;
